    ...
    <script>
        ...
        function send() {
            ...
            document.getElementById('chat-body').innerHTML += '<div class="chatbox__body__message chatbox__body__message--right">' +
                '<img src="' + window.location.origin + '/Do_an_4/public/images/admin.png" alt="Picture">' +
                '<div class="clearfix"></div>' +
            ...
            xhr.open("GET", "http://localhost:"+location.port+"/Do_an_4/chat/sendAdmin/" + queries, true);
            xhr.onload = function() {
                if (xhr.readyState === 4) {
                    var status = xhr.status;
                    if (status === 500) {
                        document.getElementById('chat-body').innerHTML += '<div class="chatbox__body__message chatbox__body__message--left">' +
                            '<img src="' + window.location.origin + '/Do_an_4/public/images/cry-sad.gif" alt="Picture">' +
                            '<div class="clearfix"></div>' +
                            ' <div class="ul_section_full">' +
                            ' <ul class="ul_msg">' +
        ...
        function loadData(e) {
            ...
            var xhr = new XMLHttpRequest();
            xhr.open("GET", "http://localhost:"+location.port+"/Do_an_4/chat/chating/" + e, true);
            xhr.onload = function() {
                if (xhr.readyState === 4) {
                    document.getElementById('chat-body').innerHTML = "";
                        ...
                        if (res.length > 0) {
                            for (let index = 0; index < res.length; index++) {
                                if (res[index].fromUserId == userId) {
                                    ...
                                    document.getElementById('chat-body').innerHTML += '<div class="chatbox__body__message chatbox__body__message--left">' +
                                        '<img src="' + window.location.origin + '/Do_an_4/public/images/user.png" alt="Picture">' +
                                        '<div class="clearfix"></div>' +
                                        ' <div class="ul_section_full">' +
                                        ' <ul class="ul_msg">' +
